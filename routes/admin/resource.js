...
router.post("/add", checkSupreme, upload.array("img", 2), async (req, res) => {
    ...
    for (let file of req.files) {
        await sharp(file.filename).toFormat("jpeg").jpeg({ quality: 5, force: true }).toFile("toConvert.jpg");
...
router.get("/delete/:id", checkSupreme, async (req, res) => {
    ...
    gfs.remove({ filename: resource.dark, root: "fs" }, async (err, gridStore) => {
        if (err) {
            res.send(err);
        } else {
    ...
    gfs.remove({ filename: resource.light, root: "fs" }, async (err, gridStore) => {
        if (err) {
            res.send(err);
        }
    });
...
// EDITING
router.put("/edit/:id", checkSupreme, upload.fields([{ name: "dark", maxCount: 1 }, { name: "light", maxCount: 1 }]), async (req, res) => {
    let resource = await Resource.findById(req.params.id);
    ...
    // Compress
        for (let key of Object.keys(req.files)) {
            await sharp(req.files[key][0].filename).toFormat("jpeg").jpeg({ quality: 5, force: true }).toFile("toConvert.jpg");
            let filename = `${uuid.v4()}-${Date.now()}.jpg`;
            filenames.push(filename);
            const writeStream = gfs.createWriteStream(filename);
            await fs.createReadStream(`./toConvert.jpg`).pipe(writeStream);
